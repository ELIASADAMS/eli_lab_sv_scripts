...
function main() {
    var form = {
        ...
        message: "Replace dense pitch automation with intentional sparse geometry while preserving attacks, extrema and phrase structure",
        buttons: "OkCancel",
        widgets: [
    ...
    var tolerances = [2.5, 5, 10, 18];
    var tolerance = tolerances[parseInt(r.answers.tolerance)];
    var micro = [0, 1.5, 4, 8][parseInt(r.answers.micro)];
    var mode = parseInt(r.answers.mode);
    var preserve = parseInt(r.answers.preserve);

    // Work per NoteGroup so points remain continuous across note boundaries.
    var groups = [];
    for (var g = 0; g < notes.length; g++) {
        var parent = notes[g].getParent();
        var found = false;
        for (var x = 0; x < groups.length; x++) if (groups[x] === parent) found = true;
        if (!found) groups.push(parent);
    }

    for (var gi = 0; gi < groups.length; gi++) {
        var groupNotes = [];
        for (var n = 0; n < notes.length; n++) if (notes[n].getParent() === groups[gi]) groupNotes.push(notes[n]);
        ...
        // Reinsert note boundaries and strong local extrema. These are exactly
        // the places where an old VOCALOID-style curve benefits from explicit points.
        var important = [];
        ...
        if (mode >= 2) {
            for (var m = 1; m < unique.length - 1; m++) {
                ...
                if (jitter > 0) local[1] += rand(-jitter, jitter);
            }
        }

        if (mode >= 3) {
            // VOCALOID mode intentionally makes long flat regions slightly
            // stepped, but does not touch the actual phrase endpoints.
            for (var q = 1; q < unique.length - 1; q++) {
                var left = unique[q - 1][1];
                var right = unique[q + 1][1];
                if (Math.abs(unique[q][1] - left) < 3 && Math.abs(unique[q][1] - right) < 3 && Math.random() < (mode === 4 ? 0.65 : 0.35)) {
